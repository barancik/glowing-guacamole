...
alignments. \diff{My responsibilities included paraphrasing, filtering the 
noisy paraphrase tables and evaluation of experiments.}}
...
(See\Tref{number_of_substitutions}). Adding larger paraphrase tables is thus 
necessary. 
...
\diff{This chapter is structured as follows. First, we present several 
algorithms for lexical and phrase substitutions (\Sref{algorithm}). They all 
include three steps:}

\begin{enumerate}
\item \diff{selecting possible paraphrase candidates (\Sref{candidates}), }
\item \diff{paraphrasing, i.e. the substitution of the paraphrase candidates (\Sref{paraphrasing}),}
\item \diff{application of Depfix to fix grammatical errors caused by the paraphrasing (\Sref{depfix}).}
\end{enumerate}

\diff{These algorithms are utilized in \Sref{filtering-section} in an experiment 
with automatic filtering of the noise from the Meteor Tables. } 

\diff{Our method is independent of an evaluation metric. We evaluate the 
resulting paraphrases using the BLEU \citep{bleu} score because of its 
widespread utilization.  We show that despite the simplicity of the method, 
BLEU achieves a significant improvement in its correlation with human 
judgment using our new reference sentences on WMT12 a WMT13 data 
(\Sref{results}).}
...
\diff{For more detailed description of Depfix see \Sref{depfix}.}
...
\item[limited] \diff{after analyzing 50 randomly selected sentences, we find out that 
...
\diff{As was already discussed, the Meteor tables are voluminous, but they are 
very noisy due to their method of construction  (See \Sref{meteori}). The 
noise is particularly apparent among the multiword paraphrases -- for 
...
\diff{Among one-word paraphrases the noise is sparser, but there are still pairs like 
...
\diff{We attempt to filter the noise from the Czech Meteor tables with two different 
...
implemented for all data, but its results are inconclusive. Therefore, we 
employ only the first method in the rest of our experiments.}


\subsection{Error-analysis Based Filtering}
\diff{After paraphrasing using only the Meteor tables, rephrased sentences were 
...
\begin{itemize}
\item morphological analysis using the Morče tool \citep{morce:2007} and 
replacing of word forms with their lemmas; 
...
32,154 couples of lemmas.\diff{All examples of bad one-word paraphrases from 
\Sref{filtering-section} are removed.}

\subsection{Automatic Filtering}
\diff{Filtering strategies described in~this section consist of assigning a score 
to~each paraphrase pair. We then gradually remove paraphrases with low scores 
and measure the effect on~the final correlation of~BLEU computed on paraphrased references with human judgment.}
...
\diff{where $\text{lex}$ are lexical translation probabilities computed using
maximum likelihood estimation from single best word alignment computed on
...
%To get lexical translation probabilities, we use maximum likelihood
...
\diff{Results on the data from WMT12 and WMT13 are consistent. Paraphrasing increases 
the accuracy of the evaluation using the BLEU metric, even though the differences 
in~the WMT13 data are not as~prominent due to a much higher baseline. This is 
also reflected in~the smaller amount of substitutions (see 
