...
Furthermore, its complicated. Consider the following example from WMT15 \cite{wmt15} Russian to English task. A sentence \textit{Митинг закончился возложением цветов к закладному камню.} (`The rally ended with the placing of flowers at the foundation stone.'') has a completely different translation by all 14 competing systems.   


The direct human judgment approach is widely acknowledged as the most reliable and favored by MT developers due to its accuracy. Nevertheless, this approach is associated with significant drawbacks. It is a time-consuming and expensive process when compared to the automatic alternative. Additionally, the human judgment approach is characterized by its inherent complexity, further adding to its challenges.
